...
from filter_tags import filter_tags
...
app.config['CKEDITOR_SEVER_LOCAL'] = True
app.config['CKEDITOR_FILE_UPLOADER'] = 'upload_for_ckeditor'
app.config['CKEDITOR_HEIGHT'] = 400
app.config['CKEDITOR_WIDTH'] = 900
...
def filter_tag(string):
    res = re.sub('<([a-z])*>', '', string)
    return re.sub('</([a-z])*>', '', res)


def default_show(string):
    if len(string)==2:
        return '图片'
    else:
        return string
...
app.add_template_filter(default_show)
