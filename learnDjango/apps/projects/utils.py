...
def get_count_by_project(datas):
    ...
    # item['create_time'] = match.group(1) + " " + match.group(2)
        ...
        testcases_count = Interfaces.objects.values('id').annotate(testcases_count=Count('testcases')).filter(
            project_id=project_id, is_delete=False)
        configures_count = Interfaces.objects.values('id').annotate(configures_count=Count('configures')).filter(
            project_id=project_id, is_delete=False)
